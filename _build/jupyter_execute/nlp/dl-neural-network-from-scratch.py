...
- This lecture note provides an intuitive understanding of the mechanism of the neural network, or deep learning.
...
- In `tensorflow.kears`, we can create models in two ways:
    - Sequential API
    - Functional API
...
x = layers.Dense(4, activation="relu", name="first_layer")(inputs)
outputs = layers.Dense(2, activation="relu", name="second_layer")(x)
...
- Forward propagation shows how the network takes the input values, transforms them, and produces the predictde output values based on the network parameters (i.e., weights).
...
- Most importantly, machine learning and deep learning both requires input and output variables to be **numbers**.
...
- Given a corpus, we can create its **vocabulary** (at the word-token level).
- We can associate a **unique integer index** with every word type (i.e., integer encoding).
...
![](../images/neural-network-loss-binary.png)

- When the target class label is a multi-level one (e.g., next-word prediction):
![](../images/neural-network-loss-multicat.png)

## Examples of Loss Errors
...
### Intuition of Gradients: An Example
...
- In keras, it's an independent layer: `keras.layers.dropout()`.
